...
## Current client TypeScript tree (clean state)

The runtime client source has been intentionally reduced to a ToastSileo-only example.

```text
web/packages/client/typescript/
├── components/
│   ├── index.ts
│   ├── ToastSileo.tsx
│   └── ToastSileo/
│       ├── HtmlText.tsx
│       ├── ToastSileo.d.ts
│       ├── ToastSileoControls.tsx
│       ├── ToastSileoView.module.css
│       └── ToastSileoView.tsx
├── css/
│   └── main.css
...
- Legacy component-specific client folders/hooks/shared utilities were removed.
- `rad-client-components.ts` registers only `ToastSileoComponentMeta`.
...
- _Lerna_ - used to orchestrate the building of multiple inter-dependent node packages
- _Yarn_ - used as a dependency manager, replacing `npm` as the package manager in the context of the `web/` packages being
  built.
- _Vite_ - a build tool/bundler that combines or _bundles_ the necessary files/source into something
...
- _Typescript_ - A superset of Javascript, which allows for strong typing, fuller OOP support. Ultimately compiles
...
- _.npmrc_ - Contains configuration used by npm (as well as in our case - yarn), to determine where it resolves
...
- _vite.config.ts_ - configuration file used for Vite.
- _package.json_ - Where package names, versions, and dependencies are defined, including dev dependencies. May
  optionally contain configuration for additional tools.
- _tsconfig.json_ - Contains configuration specific to the typescript compiler as well as (optionally) configuration
  for typescript related build plugins/tools
- _yarn.lock_ - a dependency 'lock file' - which is used to 'lock' the dependency structure into specific versions and
...
- _typescript/_ - folder containing all the typescript source files. The root `<scopename>.ts` is the 'index' or
...
- _dist/_ - the `distribution` or `build output` directory. It's created and populated with the result of your
